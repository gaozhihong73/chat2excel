...
public class LogOperationAspect {
    /**
     * 用于序列化请求/响应的Jackson工具  用于在 Java 对象和 JSON 字符串之间进行转换。
     */
    private final ObjectMapper objectMapper;

    public LogOperationAspect(ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
        // 设置此项为 false 可以防止程序因为无法序列化某个对象而导致业务逻辑崩溃。它会让 Jackson 遇到空对象时返回一个空的 JSON {} 而不是直接报错。
     ...
     * 环绕通知
     */
    @Around("@annotation(com.red.common.annotation.LogOperation)")
    public Object logOperation(ProceedingJoinPoint joinPoint) {
        ...
        // 获取注解实例从当前方法上寻找 @LogOperation 注解。如果没有这个注解，此变量为 null
    ...
    private void logRequest(String operation, ProceedingJoinPoint joinPoint, HttpServletRequest request) {
        ...
        // 2. 加上访问路由和者访问方法
        if (request != null) {
            logBuilder.append("methond=").append(request.getMethod()).append(", url=").append(request.getRequestURI());
